...
6. **Teachers**（`#teachers`）— 講師紹介。講師3名とも実写真。各講師の項目は「出身校」「保有資格」「メッセージ」。メッセージ欄は改行（`\n`）を表示する
...
| 合格体験記 | 現在2件。追加・削除は管理画面、写真・推しポイントなど既存の内容の変更はSQLで行う（管理画面に編集機能が無いため） |
| コラム | 継続して記事を増やす |
...
サイトに変更を加えたら、**同じコミットでこの表に1行追加し、影響する本文の章も更新する**（新しい順）。
細かい経緯は `git log` を参照。これより前の変更は、2026-09-18 の全面更新で本文に反映済み。

| 日付 | 変更内容 |
|---|---|
| 2026-09-20 | 合格体験記（全商英検1級）の写真を本人の写真に差し替え、推しポイントを追加（写真はサイト内の画像、内容はSQLで更新） |
| 2026-09-20 | 講師紹介の項目名「自己紹介」を「メッセージ」に変更 |
| 2026-09-18 | 変更のたびに SPEC.md へ記録する運用を開始し、この「変更履歴」を追加 |
| 2026-09-18 | 使われていない管理画面の仮コンポーネント `ComingSoon.tsx` を削除 |
| 2026-09-18 | SPEC.md を現在の実装に合わせて全面更新 |
| 2026-09-18 | 合格者の声：左右の矢印を削除し、スワイプとドットで切り替えるように変更。推しポイントを任意にし、空欄時は非表示 |
| 2026-09-18 | 合格体験記を1件追加（全商英検1級、画像は仮置き） |
| 2026-09-17 | 講師（英語顧問）の写真を差し替え |
| 2026-09-17 | 管理画面の日ごとの受付時間カレンダーに予約状況を表示し、予約済みの時間を変更不可に |
| 2026-09-17 | 既存生徒用の授業申し込みページを追加（秘密のURL、無料体験と受付枠を共有） |
